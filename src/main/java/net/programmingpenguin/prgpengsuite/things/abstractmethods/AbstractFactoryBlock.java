...
public abstract class AbstractFactoryBlock extends BlockWithEntity {
    public static final DirectionProperty FACING;
    public static final BooleanProperty LIT;

    protected AbstractFactoryBlock(Settings settings) {
        super(settings);
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack itemStack) {
        if (itemStack.hasCustomName()) {
            ...
            if (blockEntity instanceof AbstractFactoryEntity) {
                ((AbstractFactoryEntity)blockEntity).setCustomName(itemStack.getName());
            }
        }
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (!state.isOf(newState.getBlock())) {
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if (blockEntity instanceof AbstractFactoryEntity) {
                ItemScatterer.spawn(world, pos, (AbstractFactoryEntity)blockEntity);
                ((AbstractFactoryEntity)blockEntity).method_27354(world, Vec3d.ofCenter(pos));
